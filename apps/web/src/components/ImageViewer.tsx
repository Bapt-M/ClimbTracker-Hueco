...
export const ImageViewer = ({ imageUrl, isOpen, onClose }: ImageViewerProps) => {
  ...
  const getCanvasCoordinates = (e: React.MouseEvent | React.Touch) => {
    ...
    // Get the bounding rect of the transformed image (after scale/translate)
    const imgRect = img.getBoundingClientRect();

    // Calculate position relative to the transformed image
    const x = e.clientX - imgRect.left;
    const y = e.clientY - imgRect.top;

    // Convert to canvas coordinates
    // The canvas has the same transform as the image, so we need to:
    // 1. Get position as ratio of displayed (transformed) size
    // 2. Apply to canvas native resolution
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    if (drawMode) {
      // En mode dessin, empêcher le déplacement
      e.preventDefault();
      startDrawing(e);
    } else {
      // En mode normal, permettre le déplacement
      setIsDragging(true);
      setDragStart({
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (drawMode) {
      // En mode dessin, seulement dessiner
      if (isDrawingPath) {
        e.preventDefault();
        draw(e);
      }
    } else if (isDragging) {
      // En mode normal, seulement déplacer
      setPosition({
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-[101] flex flex-col gap-2">
        <button
          ...
          title={drawMode ? 'Désactiver le dessin' : 'Activer le dessin'}
        >
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 text-white/70 text-sm bg-black/30 backdrop-blur-sm px-4 py-2 rounded-full text-center">
        {drawMode ? 'Mode dessin activé • Cliquez pour dessiner' : 'Glissez pour déplacer\n Molette pour zoomer'}
      </div>
    </div>
  );
};
